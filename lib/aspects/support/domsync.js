...
DomSync.prototype = {
  ...
  sync: function sync(p, struct, seq) {
    ...
    var n, ne;
    this.syncing = true;
    //console.log('sync', seq, p.id, struct);
    ...
    for (i=0; i < obsolete.length; i++) {
      ...
      if (typeof getSelection != 'undefined') {
        var s = getSelection();
        if (el == s.anchorNode || el.compareDocumentPosition(s.anchorNode) >= 10) {
          s.removeAllRanges();
          ...
          if (ne) {
            ...
            //console.log('Moving selection after', el.previousSibling);
            r.setStartAfter(el.previousSibling || el.parentNode);
          }
          s.addRange(r);
        }
      }
