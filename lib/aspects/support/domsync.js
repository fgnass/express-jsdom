...
DomSync.prototype = {
    ...
    if (el.getAttribute('data-sync') && !el.sync) {
      ...
      if (id) {
        ...
        if (orig) {
          ...
          // if(orig.innerText != el.innerText) { //TODO Use textContent in FF
  ...
  serialize: function(p, filter) {
    ...
    while (el) {
      if (el.nodeType == 3) {
        ...
        this.undupe(el);
        /*
        var ex = this.doc.getElementById(el.id);
        if (el.id && ex != el) {
          console.log('Copy detected');
          console.log('el', el.sync, el.outerHTML);
          console.log('ex', ex.sync, ex.outerHTML);
          if (el.innerHTML != ex.innerHTML) {
            el.removeAttribute('data-model');
          }
          el.removeAttribute('data-sync');
          el.removeAttribute('id');
  ...
  send: function(p, filter) {
    if (!this.syncing && p.sync) {
      ...
      if (typeof document != 'undefined') console.log('Sending', p.id, struct);
      this.socket.emit('domSync', p.id, struct);
    }
  },
