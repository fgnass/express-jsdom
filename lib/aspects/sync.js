...
exports.client = function() {
  ...
  function sendOnNextTick(el) {
    if (!~dirty.indexOf(el)) {
      dirty.push(el);
      if (!flush) {
        flush = setTimeout(flushDirtyNodes, 0);
      }
    }
  }
...
exports.apply = function($, window, document) {
  ...
  $(window).bind('clientConnect', function(ev) {
    ...
    domsync.setSocket(socket, function(elements, seq) {
      var detail = {elements: elements, seq: seq};
      elements.forEach(function(el) {
        utils.trigger(el, 'domsync', {detail: detail});
      });
    });
    ...
    $.fn.mutate = function() {
      ...
      $.client.pause();
      try {
        ret = _mutate.apply(this, arguments);
      }
      finally {
        flushDirtyNodes();
        $.client.unpause();
